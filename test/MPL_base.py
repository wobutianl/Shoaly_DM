...
class MPL_Panel_base(wx.Panel):  
    ...
    #----------------------------------------------------------------------
    def xticklabels(self, position):
        """设置x轴的显示值"""
        self.axes.set_xticklabels(position)
    #----------------------------------------------------------------------
    def xticks(self, position, value):
        """设置x轴显示值"""
        self.axes.set_xticks(position, value)
...
class MPL_Frame(wx.Frame):  
    ...
    def Button1Event(self,event):  
        ...
        self.MPL.plotImg(r"E:\lab\Paper\Data\data\dudu.jpg")
        self.MPL.plot(x,y,'--*g') 
        self.MPL.showAxis()
