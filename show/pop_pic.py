...
class MyFrame(wx.Frame):
	...
	#data =  [{'1': '\xe5\xb7\xb2\xe6\x9c\x89\xe9\x93\x81\xe8\xb7\xaf'}, {'1':'\xe5\xb7\xb2\xe6\x9c\x89\xe9\x93\x81\xe8\xb7\xaf'}
        ...
        self.MPL = MPL_base.MPL_Panel_base(self)
	...
	for cell in self.data:
	    for num in range(0, len(cell)):
		if cell.keys()[num] == u"年份":
		    label.append(cell.values()[num])
		elif cell.keys()[num] == u"年末总人口（万人）":
		    height.append(cell.values()[num])	
	    
	left = np.arange(len(label))

	self.MPL.plotBar(left, height, width = 0.5, align="center")

	self.MPL.showAxis()
	
	self.MPL.ylabel(u"单位/万人")
	self.MPL.xlabel(u"年份")
	self.MPL.xticklabels(label)
	
	self.MPL.ylim(72, 75)
	#self.MPL.xticks(left, label)

	self.MPL.title_MPL("Pop")    
	self.MPL.grid("false")   
